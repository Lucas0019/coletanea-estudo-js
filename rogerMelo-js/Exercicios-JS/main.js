...
const verificaImpar = (radomNumber) => radomNumber % 2 === 1;

const oddNumbers = randomNumbers.filter(verificaImpar);
...
const countNumbers = (accumulator, crazyNumber) =>
  crazyNumber < 501 ? ++accumulator : accumulator;

const qtosNumerosMenor501 = crazyNumbers.reduce(countNumbers, 0);

console.log(qtosNumerosMenor501); //5
...
const elevadoQuadrado = numbers.map((number) => number ** 2);
...
// const antesDe2000 = tarantinoMovies.filter(
//   (tarantinoMovie) => tarantinoMovie.release < 2000
// );

//Função Melhorada - PARTE 01
const antesDe2000 = tarantinoMovies.filter(({ release }) => release < 2000);
